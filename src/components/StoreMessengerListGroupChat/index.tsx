...
const StoreMessengerListGroupChat = (props: IStoreMessengerListGroupChatProps) => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { listGroupChat, activeGroup, setActiveGroup, setPageStatus, storeProfile, deletedMessageKeyId, socket } =
    props;
  ...
  useEffect(() => {
    if (socket) {
      socket.on('new-message', (data: IDataResponseNewMessage) => {
        if (data && data.store.storeId === storeProfile.id) {
  ...
  }, [activeGroup, storeProfile, socket]);
    ...
    return listGroup?.map((item: IGroupChatDetail) => (
      ...
      >
        {item.type === PRIVATE_GROUP_CHAT_TYPE && (
          <CommonURLImage src={item.people[USER_KEY]?.avatar} className={styles.avatar} alt="Store" />
        )}
        {item.type === PUBLIC_GROUP_CHAT_TYPE && (
          <div className={styles.avatarGroup}>
            <div className={styles.avatarShop}>
              <CommonURLImage src={item.people[STORE_KEY]?.avatar} className={styles.avatar} alt="Store" />
            </div>
            <div className={styles.avatarUser}>
              <CommonURLImage src={item.people[USER_KEY]?.avatar} className={styles.avatar} alt="User" />
            </div>
          </div>
        )}
        <div className={styles.chatInfo}>
          <div className={styles.nameStore}>
            <span>
              {item.people[USER_KEY]?.name}
              {item.type === PUBLIC_GROUP_CHAT_TYPE && `, ${item.people[STORE_KEY]?.name}`}
            </span>
          </div>
    ...
    if (listGroupChat.length) {
      setListGroup(
        listGroupChat.filter((item: IGroupChatDetail) =>
          item.people[USER_KEY]?.name?.toLowerCase().includes(nameSearch.trim().toLowerCase())
        )
      );
    }
    ...
    <div className={styles.listGroupChat}>
      <Row className={styles.rowListChatStore}>
        <Col span={24} className={styles.title}>
          <img src={backLeft} alt="backLeft" width={38} height={24} onClick={() => navigate('/store-management')} />
...
export default StoreMessengerListGroupChat;
